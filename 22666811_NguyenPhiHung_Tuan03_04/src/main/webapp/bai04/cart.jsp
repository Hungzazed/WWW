...
<div class="container mx-auto px-4 py-8">
    <h2 class="text-2xl font-bold text-gray-800 mb-6">Your Book Cart</h2>
    ...
    <c:if test="${not empty bookcart.items}">
        <div class="overflow-x-auto">
            <table class="w-full bg-white rounded-lg shadow-md">
                ...
                <tr>
                    <th class="py-3 px-4 text-left text-gray-700 font-semibold">Title</th>
                    <th class="py-3 px-4 text-left text-gray-700 font-semibold">Author</th>
                ...
                <c:forEach var="item" items="${bookcart.items}">
                    <tr class="border-b hover:bg-gray-50">
                        <td class="py-4 px-4 text-gray-800">${item.book.title}</td>
                        <td class="py-4 px-4 text-gray-800">${item.book.author}</td>
                        <td class="py-4 px-4">
                            <form action="${pageContext.request.contextPath}/bookcart" method="post" class="flex items-center space-x-2">
                        ...
                        <td class="py-4 px-4 text-gray-800">$${item.book.price}</td>
                        <td class="py-4 px-4 text-gray-800">$${item.book.price * item.quantity}</td>
                        <td class="py-4 px-4">
        ...
        <p class="mt-6 text-xl font-semibold text-gray-800"><strong>Total:</strong> $${bookcart.total}</p>
    </c:if>

    <div class="mt-6">
        <a href="books" class="inline-block text-white bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded-md transition duration-300">
            Continue Shopping
        </a>
    </div>
</div>
